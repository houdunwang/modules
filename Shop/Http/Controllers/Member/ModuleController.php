...
class ModuleController extends Controller
{
    ...
    protected function extract()
    {
        try {
            $temp = 'temp/' . auth()->id();
            $file = request()->input('zip');
            Storage::deleteDirectory($temp);
            (new Zipper())->make(trim($file,'/'))->extractTo(Storage::path($temp));
        } catch (\Exception $exception) {
            throw new ResponseHttpException('压缩包解压失败');
        }
    }
    ...
    protected function getConfig()
    {
        ...
        if (empty($dirs) || !is_file(base_path("storage/app/{$dirs[0]}/Config/package.php"))) {
            throw  new ResponseHttpException('模块文件错误');
        }
